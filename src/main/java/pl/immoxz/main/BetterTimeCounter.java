...
public class BetterTimeCounter {
    ...
    public long[] countDiff(String H, String M, String S) {
        ...
        try {
            ...
            if (setTime.getTime() > curTime.getTime()) {
                diff = setTime.getTime() - curTime.getTime();
            }else{
    ...
    public long[] CalculateTimeFromMilsec(long milisec) {
        long hours = TimeUnit.MILLISECONDS.toHours(milisec);
        long minutes = (TimeUnit.MILLISECONDS.toMinutes(milisec) - (hours * 60)) % 60;
        long seconds = (TimeUnit.MILLISECONDS.toSeconds(milisec) - (hours * 60 * 60) - (minutes * 60)) % 60;
        long[] setOfTimes = {hours, minutes, seconds};
        return setOfTimes;
    }
